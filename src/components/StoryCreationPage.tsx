...
interface StoryCreationPageProps {
  ...
  onBack: () => void;
  onCreateStory: (selectedArticleIds: string[]) => void;
  searchTerm: string;
...
export function StoryCreationPage({
  ...
  // Reset selection if initialSelectedArticleIds changes
  useEffect(() => {
    setSelectedArticleIds(initialSelectedArticleIds);
  }, [initialSelectedArticleIds]);

  // Available articles (published + pending)
  ...
  // Apply filters and search to available articles
  const filteredArticles = useMemo(() => {
    ...
    return filtered;
  }, [availableArticles, searchTerm, filterBy, quickFilters, sortBy]);
    ...
    setSelectedArticleIds(prev => {
      if (prev.includes(articleId)) {
        ...
        toast.warning("Maximum 8 articles allowed", {
          description: "You can select up to 8 articles for a story.",
          duration: 3000,
        });
        return prev;
      }
  ...
  const handleDrop = (articleId: string) => {
    ...
    if (!selectedArticleIds.includes(articleId) && selectedArticleIds.length < 8) {
      setSelectedArticleIds(prev => [...prev, articleId]);
      toast.success("Article added to story", {
      ...
      toast.info("Article already selected", {
        description: "This article is already in your story selection.",
        duration: 2000,
      });
    }
  };
  ...
  const handleContinue = () => {
    if (selectedArticleIds.length === 0) {
      toast.error("No articles selected", {
        description: "Please select at least one article for the story.",
        duration: 3000,
      });
      return;
    }

    onCreateStory(selectedArticleIds);
  };
        ...
        <div className="px-6 pt-6">
          ...
          <div className="flex items-start gap-4 mb-4">
            ...
            <div className="flex-1 min-w-0">
              <h1 className="text-xl font-medium truncate">{storyData.headline}</h1>
              <p className="text-muted-foreground text-sm mt-1">
                {storyData.category} - {storyData.subcategory}
              </p>
            </div>
        ...
        <div className="px-6 py-3 bg-blue-50 border-t">
          <div className="flex items-center justify-between text-sm">
            <span className="text-blue-700">
              Showing published and pending articles • Click checkbox or drag articles to select
            </span>
            ...
            <div className="p-4 border-b bg-white">
              ...
              <p className="text-sm text-muted-foreground mt-1">
                {selectedArticleIds.length > 0 
                  ? `${selectedArticleIds.length} of 8 selected`
                  : 'Select articles for your story'
                }
              </p>
            </div>
      ...
      <div className="flex-shrink-0 border-t bg-white p-4">
        <div className="flex items-center justify-between">
          <div className="text-sm text-muted-foreground">
            {selectedArticleIds.length} of 8 articles selected
          </div>
          <div className="flex gap-3">
            ...
            <Button 
              ...
              style={{
                backgroundColor: selectedArticleIds.length > 0 ? '#5767F2' : undefined,
                borderColor: selectedArticleIds.length > 0 ? '#2533B0' : undefined,
                color: selectedArticleIds.length > 0 ? 'white' : undefined
              }}
