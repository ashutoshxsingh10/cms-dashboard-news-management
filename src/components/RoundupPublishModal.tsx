...
interface RoundupPublishModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: (settings: RoundupPublishSettings) => void;
  roundupData: {
    name: string;
    description: string;
    type: string;
...
export interface RoundupPublishSettings {
  isFeaturedRoundup: boolean;
...
export function RoundupPublishModal({
  isOpen,
  onClose,
  onConfirm,
  roundupData,
  articleCount
}: RoundupPublishModalProps) {
  const [settings, setSettings] = useState<RoundupPublishSettings>({
    isFeaturedRoundup: false,
    startTimeMode: 'now',
    customTime: '12:00',
    durationType: 'days',
    durationValue: 1,
  ...
  const updateSetting = (key: keyof RoundupPublishSettings, value: any) => {
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-md p-0 gap-0 bg-white max-h-[90vh] overflow-y-auto">
        ...
        <DialogHeader className="p-6 pb-4">
          <DialogTitle className="text-lg font-medium">Publish Roundup Settings</DialogTitle>
          <DialogDescription className="text-sm text-muted-foreground mt-1">
            Configure publishing options for "{roundupData.name}" with {articleCount} articles.
          </DialogDescription>
        </DialogHeader>

        {/* Content */}
        <div className="px-6 pb-4 space-y-4">
          {/* Featured Roundup Toggle */}
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                <Label className="text-sm font-medium">Mark as Featured Roundup*</Label>
                <p className="text-xs text-muted-foreground">
                  *Featured roundups appear prominently on the home page
                </p>
              </div>
              <Switch
                checked={settings.isFeaturedRoundup}
                onCheckedChange={(checked) => updateSetting('isFeaturedRoundup', checked)}
              />
            </div>
          </div>
